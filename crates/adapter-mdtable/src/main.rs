...
fn id_index(config: &Config, headers: &[String]) -> Option<usize> {
...
fn read_table<'a>(
    ...
    let table_line = table
        .rows
        .first()
        .map_or(0, |row| row.line.saturating_sub(2));
...
fn build_document<'a>(config: &'a Config, target: &Path) -> Result<Document<'a>, String> {
    ...
    for (path, file) in selected_files(&mut document, target, &matcher) {
        ...
        for table in parse::tables(text) {
            read_table(&mut document, config, table, &file);
        }
    }
    Ok(document)
}

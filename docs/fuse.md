...
`lattice fuse` combines registers from different repos into one graph so you can check
the references between them. Each source still uses its own profile and adapter. You
add a manifest that lists the sources, and a fuse profile that declares the edges
between them.

`openspec/specs/program-composition/spec.md` describes the full behavior.
...
sources:
  - name: reqs
    ...
    target: ../requirements
  - name: specs
    profile: ../specs/profiles/openspec.yaml
    adapter: ../specs/adapters/openspec
    target: ../specs
```
...
Edge kinds declared in the fuse profile only match against their `allowed` pairs.
Undeclared edge kinds still resolve inside their own source. Add intra-source pairings
(e.g. `[tests:test, tests:test]`) or those edges become `VACANCY`.
...
`reqs:requirement`; pathway `stage` from `specs` becomes `specs:stage`. The original
un-prefixed form is the "raw ID". A raw ID in more than one source produces
`CROSS_SOURCE_DUPLICATE_ID`.

**3. Cross-source resolution.** For each edge whose kind the fuse profile declares,
fuse looks for a node with an allowed target kind and a matching raw ID. One match
resolves the edge. No match: `VACANCY`. Multiple matches: `AMBIGUOUS_CROSS_REF`.
